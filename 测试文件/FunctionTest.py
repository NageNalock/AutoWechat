...
def joke():
    ...
    rex = r'(?<=div class="content">).*?(?=<!--)'.encode('utf-8')
    m_8 = re.findall(rex, html_8, re.S)
    m_24 = re.findall(rex, html_24, re.S)
    m_8.extend(m_24)
    random.shuffle(m_8)
    text = m_8[0].decode('utf-8').replace('<br/>', '').replace('\n', '')
    # print text 清洗<>标签
    p = re.compile('<[^>]+>')
    # print p.sub("", text)
    output = p.sub("", text)
...
def weather(content):
    ...
    if content:
        data = json.loads(content.decode('utf-8'))  # 使用json库将字符转化为字典
        res = data["result"]
...
def youdao(word):
    ...
    # md5
    m2 = hashlib.md5()
    m2.update((appKey + query + str(salt) + key).encode('utf-8'))
    sign = m2.hexdigest()
    # print sign
    data = {
        'q': query,
